...
function StripeConnectModal({ isOpen, onClose, onSuccess, accountType = 'account_onboarding' }) {
    ...
    const handleConnectStripe = async () => {
        ...
        try {
            ...
            console.log('Modal: Calling createStripeAccountLink...');
            const response = await createStripeAccountLink(); 
            console.log('Modal: Response from createStripeAccountLink:', response);

            if (response && response.success) {
                if (response.url) {
                    console.log('Modal: Stripe link created successfully, redirecting to:', response.url);
                    setSuccess(true); 
                    setError('');
                    setInfoMessage('');
                    setTimeout(() => {
                ...
                } else if (response.onboarding_complete) {
                    console.log('Modal: Stripe onboarding already complete.');
                    setSuccess(false);
                    setError(''); 
                    setInfoMessage('Your Stripe account is already connected and onboarding is complete.'); 
                    setIsLoading(false);
                    setTimeout(() => {
                        onSuccess(); 
                        onClose();   
                    ...
                    throw new Error('Unexpected response from Stripe link creation. Please try again.');
                }
            } else {
                throw new Error(response?.message || 'Failed to create Stripe account link. No URL returned.');
            }
        ...
        <div className="stripe-connect-modal-overlay">
            <div className="stripe-connect-modal">
                <div className="stripe-connect-modal-header">
                    ...
                    <button className="close-button" onClick={onClose} disabled={isLoading || success}>×</button>
                </div>
                ...
                <div className="stripe-connect-modal-content">
                    ...
                    <div className="stripe-connect-actions">
                        <button 
                            ...
                            disabled={isLoading || success || !!infoMessage} 
                        ...
                        <button onClick={onClose} className="cancel-button" disabled={isLoading || success || !!infoMessage}>
                            Cancel
                        </button>
                    </div>
                </div>
